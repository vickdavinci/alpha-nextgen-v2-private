...
class OptionsEngine:
    ...
    def resolve_vass_direction_context(
        ...
        *,
        ...
        elif hasattr(self.algorithm, "_get_transition_execution_context"):
            try:
                ctx = dict(self.algorithm._get_transition_execution_context() or {})
            except Exception:
                ctx = self._get_regime_transition_context(regime_score)
        else:
            ctx = self._get_regime_transition_context(regime_score)
        ...
        try:
            vix_level_for_vass = float(self.algorithm._get_vix_level())
        except Exception:
            vix_level_for_vass = 20.0
        self.update_iv_sensor(vix_level_for_vass, current_date_str)
        has_conviction, conviction_direction, conviction_reason = self.get_iv_conviction()
        macro_direction = self.get_macro_direction(regime_for_vass)
        allow_macro_veto = True
        if has_conviction and conviction_direction == "BEARISH":
            allow_macro_veto, veto_reason = self.get_iv_bearish_veto_status()
            if not allow_macro_veto:
        ...
        if not should_trade:
            ...
            if "E_OVERLAY_STRESS_BULL_BLOCK" in str(resolve_reason):
                if hasattr(self.algorithm, "_diag_overlay_block_count"):
                    self.algorithm._diag_overlay_block_count = (
                        int(getattr(self.algorithm, "_diag_overlay_block_count", 0)) + 1
                    )
            return None

        current_vix = float(getattr(self.algorithm, "_current_vix", vix_level_for_vass) or 0.0)
        if (
        ...
        if (
            ...
            self.algorithm.Log(
                ...
                f"VIX={current_vix:.1f} >= {float(getattr(config, 'VASS_NEUTRAL_BULL_OVERRIDE_MAX_VIX', 20.0)):.1f} | "
        ...
        if (
            ...
            and hasattr(self.algorithm, "_is_premarket_shock_memory_active")
            and self.algorithm._is_premarket_shock_memory_active()
            and resolved_direction == "BULLISH"
        ):
            resolved_direction = "BEARISH"
            resolve_reason = f"{resolve_reason} | SHOCK_MEMORY_FORCE_BEARISH"
            shock_pct = 0.0
            if hasattr(self.algorithm, "_get_premarket_shock_memory_pct"):
                try:
                    shock_pct = float(self.algorithm._get_premarket_shock_memory_pct())
                except Exception:
                    shock_pct = 0.0
            self.algorithm.Log(
    ...
    def scan_spread_for_direction(
        ...
        *,
        ...
        if self.algorithm is None:
            return
        alg = self.algorithm
        current_vix = float(getattr(alg, "_current_vix", 20.0) or 20.0)
        overlay_state = self.get_regime_overlay_state(
        ...
        can_swing_vass, swing_reason_vass = self.can_enter_swing(
            direction=direction, overlay_state=overlay_state
        )
        if not can_swing_vass:
            if "R_SLOT_DIRECTION_OVERLAY" in swing_reason_vass:
                alg._diag_overlay_slot_block_count += 1
            alg._record_vass_reject_reason("SWING_SLOT_BLOCK")
            throttle_key = (
        ...
        if not swing_filters_ok:
            ...
            alg._record_vass_reject_reason("SWING_FILTER")
            throttle_key = (
        ...
        if len(candidate_contracts) < 2:
            alg._record_vass_reject_reason("INSUFFICIENT_CANDIDATES")
            throttle_key = (
        ...
        tradeable_eq = alg.capital_engine.calculate(alg.Portfolio.TotalPortfolioValue).tradeable_eq
        margin_remaining = alg.portfolio_router.get_effective_margin_remaining()
        if self.has_pending_spread_entry():
            alg.Log("VASS: Pending spread entry exists - skipping new spread signal")
            alg._record_signal_lifecycle_event(
                engine="VASS",
                event="DROPPED",
                signal_id=vass_signal_id,
        ...
        if signal:
            ...
            signal = alg._apply_spread_margin_guard(signal, source_tag="VASS_SPREAD")
            if signal is None:
                alg._record_signal_lifecycle_event(
                    engine="VASS",
                    event="DROPPED",
            ...
            alg._record_signal_lifecycle_event(
                ...
                direction=direction.value if direction else "",
                strategy=strategy.value if strategy else "",
                code=alg._canonical_options_reason_code(rejection_code or "UNKNOWN"),
        ...
        *,
        ...
        def _attempt_debit_route(
            ...
            if route_strategy == SpreadStrategy.BULL_CALL_DEBIT:
                qqq_open = float(getattr(alg, "_qqq_at_open", 0.0) or 0.0)
                if qqq_open <= 0:
                    try:
                        qqq_open = float(alg.Securities[alg.qqq].Open)
                    except Exception:
                        qqq_open = 0.0

                qqq_sma20 = getattr(alg, "qqq_sma20", None)
                ...
                trend_ok, trend_code, trend_detail = self.check_vass_bull_debit_trend_confirmation(
                    vix_current=float(getattr(alg, "_current_vix", 0.0) or 0.0),
                ...
                if not trend_ok:
                    self.set_last_entry_validation_failure(trend_code)
                    alg._log_high_frequency_event(
            ...
            for attempt in range(max_attempts):
                if len(pool) < 2:
                    break
                spread_legs = self.select_spread_legs_with_fallback(
                    contracts=pool,
                    direction=direction,
                ...
                route_signal = self.check_spread_entry_signal(
                    regime_score=regime_score,
                    vix_current=float(getattr(alg, "_current_vix", 0.0) or 0.0),
                    adx_value=adx_value,
                    current_price=qqq_price,
                ...
                last_validation_reason = self.pop_last_entry_validation_failure()
                retryable_quality = (
        ...
        def _attempt_credit_route(
            ...
            for attempt in range(max_attempts):
                if len(pool) < 2:
                    break
                spread_legs = self.select_credit_spread_legs_with_fallback(
                    contracts=pool,
                ...
                route_signal = self.check_credit_spread_entry_signal(
                    regime_score=regime_score,
                    vix_current=float(getattr(alg, "_current_vix", 0.0) or 0.0),
                    adx_value=adx_value,
                    current_price=qqq_price,
            ...
            if fallback_strategy is not None:
                if (
                    ...
                    if not bool(
                        ...
                        vix_now = float(getattr(alg, "_current_vix", 0.0) or 0.0)
        ...
        if last_validation_reason is not None:
            self.set_last_entry_validation_failure(last_validation_reason)
    ...
    def run_itm_intraday_explicit_cycle(
        ...
        *,
        ...
        """Run the explicit ITM intraday lane when ITM direction is proposed."""
        if self.algorithm is None:
            return
        alg = self.algorithm

        itm_weekend_cutoff_active = bool(
            getattr(alg, "_is_itm_weekend_entry_cutoff_active", lambda: False)()
        )
        if (
            ...
            itm_signal_id = (
                f"ITM-{alg.Time.strftime('%Y%m%d-%H%M')}-"
                f"{alg._diag_intraday_candidate_count + 1}"
            )
            ...
            if not itm_preflight_ok:
                ...
                alg._log_high_frequency_event(
                    ...
                    reason_key=alg._canonical_options_reason_code(itm_preflight_code),
                    message=f"INTRADAY: Blocked - {block_reason}",
                )
                drop_code = alg._canonical_options_reason_code(
                    str(itm_preflight_code or "E_PREFLIGHT_BLOCK")
                )
                drop_logged = alg._log_intraday_signal_dropped(
                ...
                if drop_logged:
                    alg._diag_intraday_dropped_count += 1
                    alg._inc_intraday_engine_counter(
                        alg._diag_intraday_dropped_by_engine,
                        IntradayStrategy.ITM_MOMENTUM,
                    )
            else:
                if alg._mark_intraday_signal_event("CANDIDATE", itm_signal_id):
                    alg._diag_intraday_candidate_count += 1
                    alg._inc_intraday_engine_counter(
                        alg._diag_intraday_candidates_by_engine,
                        IntradayStrategy.ITM_MOMENTUM,
                    )
                    alg._log_high_frequency_event(
                ...
                itm_contract = alg._select_intraday_option_contract(
                    chain,
                    itm_dir,
                    strategy=IntradayStrategy.ITM_MOMENTUM,
                ...
                if itm_contract is None:
                    ...
                    qqq_atr_value = alg.qqq_atr.Current.Value if alg.qqq_atr.IsReady else 0.0
                    itm_entry_size_multiplier = float(size_multiplier)
                    if bool(
                        getattr(config, "ITM_WEEKEND_CARRY_SIZE_HAIRCUT_ENABLED", True)
                    ) and bool(getattr(alg, "_is_itm_weekend_firewall_day", lambda: False)()):
                    ...
                    itm_signal = self.check_intraday_entry_signal(
                        vix_current=vix_intraday,
                        vix_open=alg._vix_at_open,
                        qqq_current=qqq_price,
                        qqq_open=alg._qqq_at_open,
                        current_hour=alg.Time.hour,
                        current_minute=alg.Time.minute,
                        current_time=str(alg.Time),
                        portfolio_value=effective_portfolio_value,
                        raw_portfolio_value=float(alg.Portfolio.TotalPortfolioValue),
                        best_contract=itm_contract,
                        size_multiplier=itm_entry_size_multiplier,
                        macro_regime_score=regime_score,
                        governor_scale=alg._governor_scale,
                        direction=itm_dir,
                    ...
                    if itm_signal is not None:
                        itm_signal = alg._attach_option_trace_metadata(itm_signal, source="ITM")
                        if alg._mark_intraday_signal_event("APPROVED", itm_signal_id):
                            alg.Log(
                                f"INTRADAY_SIGNAL_APPROVED: SignalId={itm_signal_id} | "
                                f"ITM_ENGINE_EXPLICIT: {itm_reason} | Direction={itm_dir.value} | "
                                f"Strategy=ITM_MOMENTUM | Contract={itm_contract.symbol}"
                            )
                            alg._diag_intraday_approved_count += 1
                            alg._inc_intraday_engine_counter(
                                alg._diag_intraday_approved_by_engine,
                                IntradayStrategy.ITM_MOMENTUM,
                            )
                            alg._record_signal_lifecycle_event(
                                engine="ITM",
                        ...
                        alg.portfolio_router.receive_signal(itm_signal)
                        alg._process_immediate_signals()
                        if itm_trace_id:
                            for rej in alg._get_recent_router_rejections():
                                if rej.trace_id == itm_trace_id and rej.source_tag.startswith(
                                    ...
                                    alg._diag_intraday_router_reject_count += 1
                                    alg.Log(
                                    ...
                                    reject_code = alg._canonical_options_reason_code(
                        ...
                        ) = self.pop_last_intraday_validation_failure(lane="ITM")
                        drop_code = alg._canonical_options_reason_code(
            ...
            alg.Log(
                ...
                f"Cutoff active at {alg.Time.strftime('%H:%M')} | "
    ...
    def run_micro_intraday_cycle(
        ...
        *,
        ...
        (
            ...
            micro_state,
            signal_reason,
        ) = self.generate_micro_intraday_signal(
            vix_current=vix_intraday,
            vix_open=alg._vix_at_open,
            qqq_current=qqq_price,
            qqq_open=alg._qqq_at_open,
            ...
            current_time=str(alg.Time),
            vix_level_override=vix_level_cboe,
            premarket_shock_pct=alg._get_premarket_shock_memory_pct(),
        )
        ...
        intraday_signal_id = (
            f"MICRO-{alg.Time.strftime('%Y%m%d-%H%M')}-" f"{alg._diag_intraday_candidate_count + 1}"
        )
        ...
        if bool(getattr(config, "ITM_ENGINE_ENABLED", False)):
            itm_dir, itm_reason = self.get_itm_direction_proposal(
                qqq_current=qqq_price,
                transition_ctx=transition_ctx,
            )
        ...
        if not should_trade:
            alg._log_high_frequency_event(
                config_flag="LOG_INTRADAY_BLOCKED_BACKTEST_ENABLED",
                category="INTRADAY_BLOCKED",
                reason_key=alg._canonical_options_reason_code(signal_reason),
            ...
            if not micro_intraday_cooldown_active and signal_reason != "R_COOLDOWN_INTRADAY_MICRO":
                retry_payload = alg._consume_intraday_retry("MICRO")
            if retry_payload is not None:
                ...
                retry_strategy = self.get_last_intraday_strategy()
                ...
                alg.Log(f"INTRADAY_RETRY: {signal_reason} | Strategy={retry_strategy.value}")
        else:
            if (
                intraday_direction == OptionDirection.CALL
                and alg._is_premarket_ladder_call_block_active()
            ):
                intraday_direction = None
                signal_reason = f"PREMARKET_LADDER_CALL_BLOCK: {alg._premarket_vix_ladder_reason}"
                alg._log_high_frequency_event(
                    config_flag="LOG_INTRADAY_BLOCKED_BACKTEST_ENABLED",
                    category="INTRADAY_BLOCKED",
                ...
                candidate_strategy = forced_intraday_strategy or self.get_last_intraday_strategy()
                ...
                if not preflight_ok:
                    ...
                    alg._log_high_frequency_event(
                        config_flag="LOG_INTRADAY_BLOCKED_BACKTEST_ENABLED",
                        category="INTRADAY_BLOCKED",
                        reason_key=alg._canonical_options_reason_code(preflight_code),
                        message=f"INTRADAY: Blocked - {signal_reason}",
                    )
                    drop_code = alg._canonical_options_reason_code(
                    ...
                    if drop_logged:
                        ...
                        alg._inc_micro_dte_counter(alg._diag_micro_dte_dropped, None)
                        alg._record_micro_drop_reason_dte(drop_code, None)
                elif alg._mark_intraday_signal_event("CANDIDATE", intraday_signal_id):
                    alg._diag_intraday_candidate_count += 1
                    alg._inc_intraday_engine_counter(
                        alg._diag_intraday_candidates_by_engine,
                        candidate_strategy,
                    ...
                    alg._record_signal_lifecycle_event(
                        engine=alg._intraday_engine_bucket_from_strategy(candidate_strategy),
                        event="CANDIDATE",
                        signal_id=intraday_signal_id,
        ...
        if intraday_direction is None:
            ...
            alg.Log(
                f"INTRADAY: Proceeding with ladder size mult {alg._premarket_vix_size_mult:.2f}"
            )
            intraday_strategy = (
                intraday_strategy or forced_intraday_strategy or self.get_last_intraday_strategy()
            )
            intraday_contract = alg._select_intraday_option_contract(
                chain,
                intraday_direction,
            ...
            if intraday_contract is None:
                ...
                if drop_logged:
                    ...
                    alg._inc_micro_dte_counter(alg._diag_micro_dte_dropped, None)
                    alg._record_micro_drop_reason_dte("E_NO_CONTRACT_SELECTED", None)
        ...
        if intraday_contract is not None and (
            ...
            if drop_logged:
                ...
                alg._inc_micro_dte_counter(
                    alg._diag_micro_dte_dropped,
                    getattr(intraday_contract, "days_to_expiry", None),
                )
                alg._record_micro_drop_reason_dte(
                    "E_BID_ASK_INVALID",
                    getattr(intraday_contract, "days_to_expiry", None),
                )
            intraday_contract = None
        ...
        if intraday_contract is not None:
            alg._inc_micro_dte_counter(
                alg._diag_micro_dte_candidates,
                getattr(intraday_contract, "days_to_expiry", None),
            )
            qqq_atr_value = alg.qqq_atr.Current.Value if alg.qqq_atr.IsReady else 0.0
            intraday_signal = self.check_intraday_entry_signal(
                vix_current=vix_intraday,
                vix_open=alg._vix_at_open,
                qqq_current=qqq_price,
                qqq_open=alg._qqq_at_open,
            ...
            if intraday_signal:
                intraday_signal = alg._attach_option_trace_metadata(intraday_signal, source="MICRO")
                if alg._mark_intraday_signal_event("APPROVED", intraday_signal_id):
                    alg.Log(
                    ...
                    alg._record_signal_lifecycle_event(
                        engine=alg._intraday_engine_bucket_from_strategy(intraday_strategy),
                        event="APPROVED",
                        signal_id=intraday_signal_id,
                ...
                alg._inc_micro_dte_counter(
                    alg._diag_micro_dte_approved,
                    getattr(intraday_contract, "days_to_expiry", None),
                )
                intraday_trace_id = (
                    intraday_signal.metadata.get("trace_id", "") if intraday_signal.metadata else ""
                )
                alg.portfolio_router.receive_signal(intraday_signal)
                alg._process_immediate_signals()
                alg._clear_intraday_retry("MICRO")
                if intraday_trace_id:
                    for rej in alg._get_recent_router_rejections():
                        if rej.trace_id == intraday_trace_id and rej.source_tag.startswith("MICRO"):
                            ...
                            self.cancel_pending_intraday_entry(
                                engine=self._intraday_engine_lane_from_strategy(
                ...
                if should_trade:
                    ...
                    ) = self.pop_last_intraday_validation_failure(lane="MICRO")
                    (
                        can_retry_now,
                        retry_reason_now,
                    ) = self.can_enter_single_leg()
                    ...
                    elif alg._margin_cb_in_progress or alg._margin_call_cooldown_until:
                        drop_code = "R_MARGIN_CB_ACTIVE"
                    elif intraday_strategy is not None and self.has_intraday_position(
                        engine=self._intraday_engine_lane_from_strategy(intraday_strategy.value)
                    ):
                    ...
                    drop_code = alg._canonical_options_reason_code(drop_code)
                    drop_logged = alg._log_intraday_signal_dropped(
                        signal_id=intraday_signal_id,
                        code=drop_code,
                        reason=signal_reason,
                    ...
                    if drop_code in {
                        ...
                        retry_expires = alg.Time + timedelta(minutes=20)
                        alg._queue_intraday_retry(
                            lane="MICRO",
                            direction=intraday_direction,
