...
def check_credit_spread_entry_signal_impl(
    ...
    # V12.18: Apply the same enforce/profile resolver used by debit path, so
    # low-VIX relaxed profile is informational unless assignment gate is active.
    if (
        strategy == SpreadStrategy.BULL_PUT_CREDIT
        and config.BEAR_PUT_ENTRY_GATE_ENABLED
        and short_leg_contract is not None
        ...
        if (
            ...
            if regime_score <= relax_regime_max and vix_current <= relax_vix_max:
                enforce_assignment_gate = False
                self.log(
                    "CREDIT_SPREAD: BULL_PUT assignment stress relax active | "
                    f"Regime={regime_score:.1f} <= {relax_regime_max:.1f} | "
                    f"VIX={vix_current:.1f} <= {relax_vix_max:.1f}",
                    trades_only=True,
                )
