    ...
    def evaluate_entry(
        ...
        *,
        ...
        if not trade_date:
            ...
            return False, "E_ITM_V2_NO_DATE", "DATE_PARSE"
        ...
        if (current_hour * 60 + current_minute) < (dec_h * 60 + dec_m):
            self._count("ITM_V2_Blocked_Other")
            return False, "E_ITM_V2_DECISION_TIME", f"{current_hour:02d}:{current_minute:02d}"

        if sma20_value is None or sma20_value <= 0:
            self._count("ITM_V2_Blocked_Trend")
            return False, "E_ITM_V2_NO_SMA20", ""
        ...
        if lower <= qqq_current <= upper:
            self._count("ITM_V2_Blocked_Trend")
            return False, "E_ITM_V2_TREND_NEUTRAL", f"Band={band:.2%}"
        ...
        if direction == OptionDirection.CALL:
            if vix_current >= call_max_vix:
                ...
                return False, "E_ITM_V2_VIX_CALL_MAX", f"VIX={vix_current:.1f}>={call_max_vix:.1f}"
            if req_vix20_falling and vix_current >= call_low_pref:
                if vix20_change is None:
                    self._count("ITM_V2_Blocked_VIX")
                    return False, "E_ITM_V2_VIX20_MISSING", ""
                if vix20_change > 0:
                    self._count("ITM_V2_Blocked_VIX")
                    return False, "E_ITM_V2_VIX20_RISING_CALL", f"VIX20d={vix20_change:+.1%}"
        else:
            if vix_current < put_min_vix or vix_current >= put_max_vix:
                self._count("ITM_V2_Blocked_VIX")
                return False, "E_ITM_V2_VIX_PUT_BOUNDS", f"VIX={vix_current:.1f}"
        ...
        if bool(getattr(config, "ITM_V2_BLOCK_SAME_DAY_SAME_DIRECTION_REENTRY", True)):
            ...
            if self._last_exit_date_by_direction.get(dir_key) == trade_date:
                ...
                return False, "E_ITM_V2_REENTRY_SAME_DAY", dir_key

        if self._pause_until and trade_date <= str(self._pause_until):
            self._count("ITM_V2_Blocked_Breaker")
            return False, "E_ITM_V2_BREAKER_GLOBAL", f"Until={self._pause_until}"
        dir_pause_until = (
        ...
        if dir_pause_until and trade_date <= str(dir_pause_until):
            self._count("ITM_V2_Blocked_Breaker")
            return False, "E_ITM_V2_BREAKER_DIRECTIONAL", f"Until={dir_pause_until}"

        dd_ok, dd_detail = self._drawdown_allows(trade_date, float(portfolio_value))
        if not dd_ok:
            self._count("ITM_V2_Blocked_Drawdown")
            return False, "E_ITM_V2_DRAWDOWN", dd_detail
    ...
    def on_trade_closed(
        ...
        *,
        ...
        if self._consecutive_losses >= 5:
            until = datetime.strptime(trade_date, "%Y-%m-%d").date() + timedelta(days=max(1, b5))
            self._pause_until = until.isoformat()
            self._log(
            ...
            until = datetime.strptime(trade_date, "%Y-%m-%d").date() + timedelta(days=max(1, b3))
            self._pause_until = until.isoformat()
            self._log(
                f"ITM_V2_BREAKER_GLOBAL|Losses={self._consecutive_losses}|Until={self._pause_until}",
                trades_only=True,
        ...
        if bool(getattr(config, "ITM_V2_DIRECTIONAL_BREAKER_ENABLED", True)):
            ...
            if direction_key == "CALL" and self._call_consecutive_losses >= 3:
                until = datetime.strptime(trade_date, "%Y-%m-%d").date() + timedelta(
                    days=max(1, dir_days)
                )
                self._call_pause_until = until.isoformat()
                self._log(
            ...
            if direction_key == "PUT" and self._put_consecutive_losses >= 3:
                ...
                self._put_pause_until = until.isoformat()
                self._log(
