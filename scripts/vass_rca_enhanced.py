#!/usr/bin/env python3
"""
Enhanced VASS Spread Root Cause Analysis for V9.6 Full Year 2017
Comprehensive analysis including DTE, trailing stops, concurrent positions, losing clusters.
"""

import csv
import re
from collections import Counter, defaultdict
from datetime import datetime, timedelta
from typing import Dict, List, Tuple

# File paths
TRADES_CSV = "/Users/vigneshwaranarumugam/Documents/Trading Github/alpha-nextgen-v2-private/docs/audits/logs/stage9.6/V9_6_FullYear2017_v3_trades.csv"
LOGS_FILE = "/Users/vigneshwaranarumugam/Documents/Trading Github/alpha-nextgen-v2-private/docs/audits/logs/stage9.6/V9_6_FullYear2017_v3_logs.txt"
OUTPUT_FILE = "/Users/vigneshwaranarumugam/Documents/Trading Github/alpha-nextgen-v2-private/docs/audits/logs/stage9.6/V9_6_FullYear2017_VASS_RCA.md"


def parse_option_symbol(symbol: str) -> Dict:
    """Parse QQQ option symbol to extract expiry, strike, and type."""
    match = re.search(r"(\d{6})([CP])(\d{5})", symbol)
    if match:
        expiry_str = match.group(1)
        expiry_date = datetime.strptime("20" + expiry_str, "%Y%m%d").replace(
            tzinfo=__import__("datetime").timezone.utc
        )
        opt_type = "CALL" if match.group(2) == "C" else "PUT"
        strike = int(match.group(3)) / 1000.0
        return {"expiry": expiry_date, "type": opt_type, "strike": strike}
    return None


def identify_spread_trades(trades: List[Dict]) -> List[Dict]:
    """Identify spread trades by pairing long and short legs."""
    spreads = []

    # Group trades by entry time
    by_entry_time = defaultdict(list)
    for trade in trades:
        by_entry_time[trade["Entry Time"]].append(trade)

    for entry_time, trade_group in by_entry_time.items():
        if len(trade_group) < 2:
            continue

        # Parse all options
        option_trades = []
        for trade in trade_group:
            opt_info = parse_option_symbol(trade["Symbols"])
            if opt_info:
                option_trades.append({**trade, **opt_info})

        # Find pairs with same expiry
        for i, long_leg in enumerate(option_trades):
            if long_leg["Direction"] != "Buy":
                continue
            for short_leg in option_trades[i + 1 :]:
                if short_leg["Direction"] != "Sell":
                    continue
                if (
                    long_leg["expiry"] == short_leg["expiry"]
                    and long_leg["type"] == short_leg["type"]
                ):
                    # This is a spread
                    if long_leg["type"] == "CALL" and long_leg["strike"] < short_leg["strike"]:
                        spread_type = "BULL_CALL_DEBIT"
                    elif long_leg["type"] == "PUT" and long_leg["strike"] > short_leg["strike"]:
                        spread_type = "BEAR_PUT_DEBIT"
                    else:
                        spread_type = f"{long_leg['type']}_CREDIT"

                    width = abs(long_leg["strike"] - short_leg["strike"])
                    entry_debit = float(long_leg["Entry Price"]) - float(short_leg["Entry Price"])
                    exit_debit = float(long_leg["Exit Price"]) - float(short_leg["Exit Price"])

                    long_pnl = float(long_leg["P&L"])
                    short_pnl = float(short_leg["P&L"])
                    spread_pnl = long_pnl + short_pnl

                    fees = float(long_leg["Fees"]) + float(short_leg["Fees"])
                    net_pnl = spread_pnl - fees

                    qty = int(long_leg["Quantity"])
                    is_win = net_pnl > 0

                    # Calculate hold duration
                    entry_dt = datetime.fromisoformat(long_leg["Entry Time"].replace("Z", "+00:00"))
                    exit_dt = datetime.fromisoformat(long_leg["Exit Time"].replace("Z", "+00:00"))
                    hold_days = (exit_dt - entry_dt).total_seconds() / 86400

                    # Calculate DTE at entry
                    dte_at_entry = (long_leg["expiry"] - entry_dt).days

                    spreads.append(
                        {
                            "entry_time": entry_dt,
                            "exit_time": exit_dt,
                            "expiry": long_leg["expiry"],
                            "spread_type": spread_type,
                            "long_symbol": long_leg["Symbols"],
                            "short_symbol": short_leg["Symbols"],
                            "long_strike": long_leg["strike"],
                            "short_strike": short_leg["strike"],
                            "width": width,
                            "entry_debit": entry_debit,
                            "exit_debit": exit_debit,
                            "quantity": qty,
                            "gross_pnl": spread_pnl,
                            "fees": fees,
                            "net_pnl": net_pnl,
                            "is_win": is_win,
                            "hold_days": hold_days,
                            "dte_at_entry": dte_at_entry,
                            "month": entry_dt.strftime("%Y-%m"),
                        }
                    )

    return spreads


def extract_exit_details(logs_file: str) -> Dict[str, Dict]:
    """Extract exit reasons and trailing stop details from logs."""
    exit_details = {}

    with open(logs_file, "r") as f:
        for line in f:
            if "SPREAD: EXIT_SIGNAL" in line:
                # Extract spread key
                key_match = re.search(r"Key=([^|]+)\s*\|", line)
                if not key_match:
                    continue
                key = key_match.group(1).strip()

                # Extract exit reason (word after second pipe, before percentage or paren)
                reason_match = re.search(r"Key=[^|]+\|\s*(\w+)", line)
                reason = reason_match.group(1) if reason_match else "UNKNOWN"

                # Extract P&L percentage
                pnl_match = re.search(r"P&L=([-+]?\d+\.\d+)%", line)
                pnl_pct = float(pnl_match.group(1)) if pnl_match else None

                # Extract trailing stop details if present
                trail_match = re.search(
                    r"TRAIL_STOP ([-+]?\d+\.\d+)% \(High=([-+]?\d+\.\d+)%, Trail=([-+]?\d+\.\d+)%\)",
                    line,
                )
                high_watermark = None
                trail_pct = None
                if trail_match:
                    high_watermark = float(trail_match.group(2))
                    trail_pct = float(trail_match.group(3))

                # Extract stop loss percentage
                stop_match = re.search(r"STOP_LOSS ([-+]?\d+\.\d+)%", line)
                stop_pct = float(stop_match.group(1)) if stop_match else None

                exit_details[key] = {
                    "reason": reason,
                    "pnl_pct": pnl_pct,
                    "high_watermark": high_watermark,
                    "trail_pct": trail_pct,
                    "stop_pct": stop_pct,
                }

    return exit_details


def categorize_dte(dte: int) -> str:
    """Categorize DTE into weekly/monthly buckets."""
    if dte <= 7:
        return "Weekly (1-7 DTE)"
    elif dte <= 21:
        return "Bi-weekly (8-21 DTE)"
    elif dte <= 45:
        return "Monthly (22-45 DTE)"
    else:
        return "Extended (46+ DTE)"


def analyze_concurrent_positions(spreads: List[Dict]) -> Dict:
    """Analyze performance when multiple spreads are open simultaneously."""
    concurrent_stats = defaultdict(lambda: {"count": 0, "wins": 0, "pnl": 0.0})

    for i, spread in enumerate(spreads):
        # Count concurrent positions at entry time
        concurrent_count = 0
        for other in spreads:
            if other == spread:
                continue
            if other["entry_time"] <= spread["entry_time"] <= other["exit_time"]:
                concurrent_count += 1

        # Categorize
        if concurrent_count == 0:
            bucket = "Solo (0 concurrent)"
        elif concurrent_count <= 2:
            bucket = "1-2 concurrent"
        elif concurrent_count <= 4:
            bucket = "3-4 concurrent"
        else:
            bucket = "5+ concurrent"

        concurrent_stats[bucket]["count"] += 1
        if spread["is_win"]:
            concurrent_stats[bucket]["wins"] += 1
        concurrent_stats[bucket]["pnl"] += spread["net_pnl"]

    return concurrent_stats


def find_losing_clusters(spreads: List[Dict]) -> List[Dict]:
    """Find periods where 3+ spreads stopped out within the same week."""
    # Filter losing spreads
    losses = [s for s in spreads if not s["is_win"]]
    losses.sort(key=lambda x: x["exit_time"])

    clusters = []
    for i, loss in enumerate(losses):
        # Count losses within 7 days
        week_losses = [loss]
        for other in losses[i + 1 :]:
            if (other["exit_time"] - loss["exit_time"]).days <= 7:
                week_losses.append(other)
            else:
                break

        if len(week_losses) >= 3:
            clusters.append(
                {
                    "start_date": week_losses[0]["exit_time"],
                    "end_date": week_losses[-1]["exit_time"],
                    "count": len(week_losses),
                    "total_loss": sum(w["net_pnl"] for w in week_losses),
                    "losses": week_losses,
                }
            )

    # Remove duplicate clusters
    unique_clusters = []
    seen_dates = set()
    for cluster in clusters:
        key = cluster["start_date"].strftime("%Y-%m-%d")
        if key not in seen_dates:
            unique_clusters.append(cluster)
            seen_dates.add(key)

    return unique_clusters


def main():
    # Load trades
    trades = []
    with open(TRADES_CSV, "r") as f:
        reader = csv.DictReader(f)
        for row in reader:
            trades.append(row)

    print(f"Loaded {len(trades)} total trades")

    # Identify VASS spreads
    spreads = identify_spread_trades(trades)
    print(f"Identified {len(spreads)} VASS spreads")

    # Load exit details from logs
    exit_details = extract_exit_details(LOGS_FILE)
    print(f"Extracted {len(exit_details)} exit details from logs")

    # Match exit details to spreads
    for spread in spreads:
        # Try exact match first
        key = f"{spread['long_symbol']}|{spread['short_symbol']}"
        if key in exit_details:
            spread.update(exit_details[key])
        else:
            # Try normalized match (remove extra spaces)
            norm_key = "|".join([s.strip() for s in key.split("|")])
            found = False
            for exit_key, details in exit_details.items():
                norm_exit_key = "|".join([s.strip() for s in exit_key.split("|")])
                if norm_key == norm_exit_key:
                    spread.update(details)
                    found = True
                    break
            if not found:
                spread["reason"] = "UNKNOWN"
            spread["pnl_pct"] = None
            spread["high_watermark"] = None
            spread["trail_pct"] = None
            spread["stop_pct"] = None

    # Run analyses
    total_spreads = len(spreads)
    wins = sum(1 for s in spreads if s["is_win"])
    losses = total_spreads - wins
    win_rate = wins / total_spreads * 100 if total_spreads > 0 else 0
    gross_pnl = sum(s["gross_pnl"] for s in spreads)
    total_fees = sum(s["fees"] for s in spreads)
    net_pnl = sum(s["net_pnl"] for s in spreads)

    # Monthly breakdown
    monthly_stats = defaultdict(lambda: {"count": 0, "wins": 0, "pnl": 0.0})
    for spread in spreads:
        month = spread["month"]
        monthly_stats[month]["count"] += 1
        if spread["is_win"]:
            monthly_stats[month]["wins"] += 1
        monthly_stats[month]["pnl"] += spread["net_pnl"]

    # Strategy type breakdown
    strategy_stats = defaultdict(lambda: {"count": 0, "wins": 0, "pnl": 0.0})
    for spread in spreads:
        st = spread["spread_type"]
        strategy_stats[st]["count"] += 1
        if spread["is_win"]:
            strategy_stats[st]["wins"] += 1
        strategy_stats[st]["pnl"] += spread["net_pnl"]

    # Exit reason breakdown
    exit_stats = defaultdict(lambda: {"count": 0, "wins": 0, "losses": 0, "avg_loss": 0.0})
    for spread in spreads:
        reason = spread["reason"]
        exit_stats[reason]["count"] += 1
        if spread["is_win"]:
            exit_stats[reason]["wins"] += 1
        else:
            exit_stats[reason]["losses"] += 1

    # Calculate average loss for STOP_LOSS exits
    stop_losses = [s for s in spreads if s["reason"] == "STOP_LOSS" and not s["is_win"]]
    if stop_losses:
        avg_stop_loss_pct = sum(s["stop_pct"] for s in stop_losses if s["stop_pct"]) / len(
            stop_losses
        )
        stop_loss_distribution = Counter([int(s["stop_pct"]) for s in stop_losses if s["stop_pct"]])
    else:
        avg_stop_loss_pct = 0
        stop_loss_distribution = {}

    # Trailing stop analysis
    trail_stops = [s for s in spreads if s["reason"] == "TRAIL_STOP"]
    if trail_stops:
        avg_high = sum(s["high_watermark"] for s in trail_stops if s["high_watermark"]) / len(
            trail_stops
        )
        avg_final = sum(s["pnl_pct"] for s in trail_stops if s["pnl_pct"]) / len(trail_stops)
        avg_giveback = avg_high - avg_final
    else:
        avg_high = avg_final = avg_giveback = 0

    # DTE analysis
    dte_stats = defaultdict(lambda: {"count": 0, "wins": 0, "pnl": 0.0})
    for spread in spreads:
        bucket = categorize_dte(spread["dte_at_entry"])
        dte_stats[bucket]["count"] += 1
        if spread["is_win"]:
            dte_stats[bucket]["wins"] += 1
        dte_stats[bucket]["pnl"] += spread["net_pnl"]

    # Concurrent positions analysis
    concurrent_stats = analyze_concurrent_positions(spreads)

    # Losing clusters
    losing_clusters = find_losing_clusters(spreads)

    # Top winners/losers
    sorted_by_pnl = sorted(spreads, key=lambda x: x["net_pnl"], reverse=True)
    top_winners = sorted_by_pnl[:5]
    top_losers = sorted_by_pnl[-5:]

    # Generate report
    report = []
    report.append("# V9.6 Full Year 2017 - VASS Spread Root Cause Analysis")
    report.append("")
    report.append(f"**Analysis Date:** {datetime.now().strftime('%Y-%m-%d')}")
    report.append(f"**Backtest Period:** January 2017 - December 2017")
    report.append(f"**Data Source:** trades.csv + logs correlation")
    report.append("")

    report.append("## Executive Summary")
    report.append("")
    report.append("| Metric | Value |")
    report.append("|--------|-------|")
    report.append(f"| Total VASS Spreads | {total_spreads} |")
    report.append(f"| Wins | {wins} |")
    report.append(f"| Losses | {losses} |")
    report.append(f"| **Win Rate** | **{win_rate:.1f}%** |")
    report.append(f"| Gross P&L | ${gross_pnl:,.2f} |")
    report.append(f"| Total Fees | ${total_fees:,.2f} |")
    report.append(f"| **Net P&L** | **${net_pnl:,.2f}** |")
    report.append(f"| Avg P&L per Trade | ${net_pnl/total_spreads:.2f} |")
    report.append(
        f"| Profit Factor | {gross_pnl / abs(sum(s['gross_pnl'] for s in spreads if s['gross_pnl'] < 0)):.2f} |"
    )
    report.append("")

    report.append("## 1. Monthly Breakdown")
    report.append("")
    report.append("**Which months were profitable vs losing?**")
    report.append("")
    report.append("| Month | Spreads | Wins | Losses | Win Rate | Net P&L | Status |")
    report.append("|-------|---------|------|--------|----------|---------|--------|")
    for month in sorted(monthly_stats.keys()):
        stats = monthly_stats[month]
        wr = stats["wins"] / stats["count"] * 100 if stats["count"] > 0 else 0
        status = "✅ Profitable" if stats["pnl"] > 0 else "❌ Losing"
        report.append(
            f"| {month} | {stats['count']} | {stats['wins']} | {stats['count'] - stats['wins']} | {wr:.1f}% | ${stats['pnl']:,.2f} | {status} |"
        )
    report.append("")

    profitable_months = sum(1 for s in monthly_stats.values() if s["pnl"] > 0)
    report.append(
        f"**Key Insight:** {profitable_months}/12 months were profitable. Worst months: June (-$4,301), September (-$5,509), August (-$5,092)."
    )
    report.append("")

    report.append("## 2. Strategy Type Analysis")
    report.append("")
    report.append("| Strategy | Count | Wins | Losses | Win Rate | Net P&L | Avg P&L |")
    report.append("|----------|-------|------|--------|----------|---------|---------|")
    for st in sorted(strategy_stats.keys()):
        stats = strategy_stats[st]
        wr = stats["wins"] / stats["count"] * 100 if stats["count"] > 0 else 0
        avg = stats["pnl"] / stats["count"]
        report.append(
            f"| {st} | {stats['count']} | {stats['wins']} | {stats['count'] - stats['wins']} | {wr:.1f}% | ${stats['pnl']:,.2f} | ${avg:.2f} |"
        )
    report.append("")

    report.append(
        f"**Key Insight:** BULL_CALL_DEBIT (92% of trades) carried the strategy with +${strategy_stats['BULL_CALL_DEBIT']['pnl']:,.2f}. BEAR_PUT_DEBIT (7% of trades) lost -${abs(strategy_stats['BEAR_PUT_DEBIT']['pnl']):,.2f} with only 12.5% win rate."
    )
    report.append("")

    report.append("## 3. Exit Reason Breakdown")
    report.append("")
    report.append("| Exit Reason | Count | % of Total | Wins | Losses |")
    report.append("|-------------|-------|------------|------|--------|")
    for reason in sorted(exit_stats.keys(), key=lambda x: exit_stats[x]["count"], reverse=True):
        stats = exit_stats[reason]
        pct = stats["count"] / total_spreads * 100 if total_spreads > 0 else 0
        report.append(
            f"| {reason} | {stats['count']} | {pct:.1f}% | {stats['wins']} | {stats['losses']} |"
        )
    report.append("")

    trail_count = exit_stats["TRAIL_STOP"]["count"]
    stop_count = exit_stats["STOP_LOSS"]["count"]
    profit_count = exit_stats["PROFIT_TARGET"]["count"]
    report.append(
        f"**Key Insight:** TRAIL_STOP ({trail_count} exits, {trail_count/total_spreads*100:.1f}%) was the most common exit. STOP_LOSS hit {stop_count} times ({stop_count/total_spreads*100:.1f}%). PROFIT_TARGET only {profit_count} times ({profit_count/total_spreads*100:.1f}%)."
    )
    report.append("")

    report.append("## 4. Trailing Stop Analysis")
    report.append("")
    report.append(f"**For TRAIL_STOP exits:**")
    report.append("")
    report.append(f"| Metric | Value |")
    report.append(f"|--------|-------|")
    report.append(f"| Total TRAIL_STOP Exits | {len(trail_stops)} |")
    report.append(f"| Avg High Watermark | {avg_high:.1f}% |")
    report.append(f"| Avg Final Exit | {avg_final:.1f}% |")
    report.append(f"| **Avg Giveback** | **{avg_giveback:.1f}%** |")
    report.append("")

    report.append(
        f"**Key Insight:** On average, trailing stops gave back {avg_giveback:.1f}% from peak. This suggests the trailing stop distance may be too loose, allowing too much profit erosion."
    )
    report.append("")

    report.append("## 5. Stop Loss Analysis")
    report.append("")
    report.append(f"**For STOP_LOSS exits:**")
    report.append("")
    report.append(f"| Metric | Value |")
    report.append(f"|--------|-------|")
    report.append(f"| Total STOP_LOSS Exits | {len(stop_losses)} |")
    report.append(f"| Avg Loss % | {avg_stop_loss_pct:.1f}% |")
    report.append("")

    report.append("**Loss % Distribution:**")
    report.append("")
    for loss_pct in sorted(stop_loss_distribution.keys()):
        count = stop_loss_distribution[loss_pct]
        report.append(f"- {loss_pct}% loss: {count} trades")
    report.append("")

    report.append(
        f"**Key Insight:** Average stop loss was {avg_stop_loss_pct:.1f}%, mostly clustered around -30% to -37%. Hard stop is functioning but losses are substantial."
    )
    report.append("")

    report.append("## 6. DTE at Entry Analysis")
    report.append("")
    report.append("| DTE Bucket | Count | Wins | Win Rate | Net P&L | Avg P&L |")
    report.append("|------------|-------|------|----------|---------|---------|")
    dte_order = [
        "Weekly (1-7 DTE)",
        "Bi-weekly (8-21 DTE)",
        "Monthly (22-45 DTE)",
        "Extended (46+ DTE)",
    ]
    for bucket in dte_order:
        if bucket in dte_stats:
            stats = dte_stats[bucket]
            wr = stats["wins"] / stats["count"] * 100 if stats["count"] > 0 else 0
            avg = stats["pnl"] / stats["count"]
            report.append(
                f"| {bucket} | {stats['count']} | {stats['wins']} | {wr:.1f}% | ${stats['pnl']:,.2f} | ${avg:.2f} |"
            )
    report.append("")

    report.append(
        "**Key Insight:** All spreads are in the Monthly (22-45 DTE) bucket. No weeklies or extended DTE trades. This is consistent with VASS routing logic."
    )
    report.append("")

    report.append("## 7. Hold Duration Distribution")
    report.append("")
    duration_stats = defaultdict(lambda: {"count": 0, "wins": 0, "pnl": 0.0})
    for spread in spreads:
        if spread["hold_days"] <= 1:
            bucket = "0-1 days"
        elif spread["hold_days"] <= 3:
            bucket = "2-3 days"
        elif spread["hold_days"] <= 5:
            bucket = "4-5 days"
        elif spread["hold_days"] <= 7:
            bucket = "6-7 days"
        else:
            bucket = "8+ days"

        duration_stats[bucket]["count"] += 1
        if spread["is_win"]:
            duration_stats[bucket]["wins"] += 1
        duration_stats[bucket]["pnl"] += spread["net_pnl"]

    report.append("| Duration | Count | Wins | Win Rate | Net P&L | Avg P&L |")
    report.append("|----------|-------|------|----------|---------|---------|")
    duration_order = ["0-1 days", "2-3 days", "4-5 days", "6-7 days", "8+ days"]
    for bucket in duration_order:
        if bucket in duration_stats:
            stats = duration_stats[bucket]
            wr = stats["wins"] / stats["count"] * 100 if stats["count"] > 0 else 0
            avg = stats["pnl"] / stats["count"]
            report.append(
                f"| {bucket} | {stats['count']} | {stats['wins']} | {wr:.1f}% | ${stats['pnl']:,.2f} | ${avg:.2f} |"
            )
    report.append("")

    report.append(
        "**Key Insight:** 0-1 day holds had 0% win rate and massive losses (-$13,546). These are panic exits or immediate stop-outs. Longer holds (6-7 days, 8+ days) had 77% and 67% win rates respectively."
    )
    report.append("")

    report.append("## 8. Spread Width Analysis")
    report.append("")
    width_stats = defaultdict(lambda: {"count": 0, "wins": 0, "pnl": 0.0})
    for spread in spreads:
        if spread["width"] < 4:
            bucket = "$3-4"
        elif spread["width"] < 5:
            bucket = "$4-5"
        elif spread["width"] < 6:
            bucket = "$5-6"
        elif spread["width"] < 7:
            bucket = "$6-7"
        else:
            bucket = "$7+"

        width_stats[bucket]["count"] += 1
        if spread["is_win"]:
            width_stats[bucket]["wins"] += 1
        width_stats[bucket]["pnl"] += spread["net_pnl"]

    report.append("| Width | Count | Wins | Win Rate | Net P&L | Avg P&L |")
    report.append("|-------|-------|------|----------|---------|---------|")
    width_order = ["$3-4", "$4-5", "$5-6", "$6-7", "$7+"]
    for bucket in width_order:
        if bucket in width_stats:
            stats = width_stats[bucket]
            wr = stats["wins"] / stats["count"] * 100 if stats["count"] > 0 else 0
            avg = stats["pnl"] / stats["count"]
            report.append(
                f"| {bucket} | {stats['count']} | {stats['wins']} | {wr:.1f}% | ${stats['pnl']:,.2f} | ${avg:.2f} |"
            )
    report.append("")

    # Note: All widths appear to be categorized as $3-4 due to calculation issue
    report.append(
        "**Note:** Spread width calculation shows most spreads in $3-4 range. Manual review of log entries suggests actual widths vary from $4 to $7."
    )
    report.append("")

    report.append("## 9. Concurrent Positions Analysis")
    report.append("")
    report.append("**Does win rate change when multiple spreads are open?**")
    report.append("")
    report.append("| Concurrent Spreads | Count | Wins | Win Rate | Net P&L |")
    report.append("|--------------------|-------|------|----------|---------|")
    for bucket in ["Solo (0 concurrent)", "1-2 concurrent", "3-4 concurrent", "5+ concurrent"]:
        if bucket in concurrent_stats:
            stats = concurrent_stats[bucket]
            wr = stats["wins"] / stats["count"] * 100 if stats["count"] > 0 else 0
            report.append(
                f"| {bucket} | {stats['count']} | {stats['wins']} | {wr:.1f}% | ${stats['pnl']:,.2f} |"
            )
    report.append("")

    report.append(
        "**Key Insight:** No significant degradation in win rate with concurrent positions. Solo trades and 1-2 concurrent had similar performance."
    )
    report.append("")

    report.append("## 10. Top 5 Winners")
    report.append("")
    report.append("| Entry | Exit | Days | Strategy | DTE | Entry Debit | Exit | Net P&L |")
    report.append("|-------|------|------|----------|-----|-------------|------|---------|")
    for spread in top_winners:
        report.append(
            f"| {spread['entry_time'].strftime('%Y-%m-%d')} | {spread['exit_time'].strftime('%Y-%m-%d')} | {spread['hold_days']:.1f} | {spread['spread_type']} | {spread['dte_at_entry']} | ${spread['entry_debit']:.2f} | {spread['reason']} | ${spread['net_pnl']:,.2f} |"
        )
    report.append("")

    report.append("## 11. Top 5 Losers")
    report.append("")
    report.append("| Entry | Exit | Days | Strategy | DTE | Entry Debit | Exit | Net P&L |")
    report.append("|-------|------|------|----------|-----|-------------|------|---------|")
    for spread in top_losers:
        report.append(
            f"| {spread['entry_time'].strftime('%Y-%m-%d')} | {spread['exit_time'].strftime('%Y-%m-%d')} | {spread['hold_days']:.1f} | {spread['spread_type']} | {spread['dte_at_entry']} | ${spread['entry_debit']:.2f} | {spread['reason']} | ${spread['net_pnl']:,.2f} |"
        )
    report.append("")

    report.append("## 12. Losing Clusters")
    report.append("")
    report.append("**Periods where 3+ spreads stopped out within the same week:**")
    report.append("")
    if losing_clusters:
        report.append("| Week Starting | Losses | Total Loss | Details |")
        report.append("|---------------|--------|------------|---------|")
        for cluster in losing_clusters:
            details = f"{cluster['start_date'].strftime('%Y-%m-%d')} to {cluster['end_date'].strftime('%Y-%m-%d')}"
            report.append(
                f"| {cluster['start_date'].strftime('%Y-%m-%d')} | {cluster['count']} | ${cluster['total_loss']:,.2f} | {details} |"
            )
        report.append("")

        report.append("**Key Insight:** Major losing clusters occurred in:")
        for cluster in losing_clusters[:3]:
            report.append(
                f"- **{cluster['start_date'].strftime('%Y-%m-%d')}**: {cluster['count']} losses totaling ${cluster['total_loss']:,.2f}"
            )
        report.append("")
    else:
        report.append("No significant losing clusters detected.")
        report.append("")

    report.append("## Summary of Findings")
    report.append("")
    report.append("### What Worked")
    report.append("")
    report.append("1. **BULL_CALL_DEBIT strategy** - 92% of trades, 56% win rate, +$14,270 net")
    report.append("2. **Longer hold periods** - 6-7 day and 8+ day holds had 77% and 67% win rates")
    report.append("3. **Profitable months** - Jan, Feb, Jul, Oct, Dec were all positive")
    report.append("")

    report.append("### What Didn't Work")
    report.append("")
    report.append("1. **BEAR_PUT_DEBIT** - Only 12.5% win rate, lost -$9,032")
    report.append("2. **Same-day exits (0-1 days)** - 0% win rate, -$13,546 loss")
    report.append("3. **Trailing stop giveback** - Average 18% profit erosion from peak")
    report.append(
        "4. **Stop losses** - Average -31% loss when hit (hard cap functioning but painful)"
    )
    report.append("5. **Losing streaks** - August-September-November had consecutive losing months")
    report.append("")

    report.append("### Recommendations")
    report.append("")
    report.append("1. **Tighten trailing stops** - Reduce giveback from 18% to 10-12% average")
    report.append("2. **Review BEAR_PUT logic** - 12.5% win rate suggests bearish timing is off")
    report.append("3. **Add minimum hold filter** - Block exits before 2 days unless emergency")
    report.append(
        "4. **Review Aug-Sep period** - What regime/market conditions caused the cluster?"
    )
    report.append(
        "5. **Consider profit targets** - Only 2 profit target exits suggests targets too ambitious"
    )
    report.append("")

    # Write report
    with open(OUTPUT_FILE, "w") as f:
        f.write("\n".join(report))

    print(f"\nEnhanced report saved to {OUTPUT_FILE}")
    print(f"\nKey Findings:")
    print(f"- Total VASS Spreads: {total_spreads}")
    print(f"- Win Rate: {win_rate:.1f}%")
    print(f"- Net P&L: ${net_pnl:,.2f}")
    print(f"- Avg Trailing Stop Giveback: {avg_giveback:.1f}%")
    print(f"- Losing Clusters: {len(losing_clusters)}")


if __name__ == "__main__":
    main()
