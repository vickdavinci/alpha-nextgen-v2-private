#!/usr/bin/env python3
"""
V10 R:R Validation Analysis Script V3
Analyzes Apr-Jun 2023 backtest data for Risk:Reward validation
Fixed: Proper Order ID matching
"""
...
def load_orders(filepath):
    """Load orders and index by row number (1-based)"""
    orders_by_id = {}
    with open(filepath, "r") as f:
        reader = csv.DictReader(f)
...
def parse_order_ids(order_ids_str):
    """Parse comma-separated order IDs, handling tabs and spaces"""
    if not order_ids_str:
        return []
    order_ids_str = order_ids_str.replace("\t", "").strip()
    return [int(x.strip()) for x in order_ids_str.split(",") if x.strip().isdigit()]
...
def get_strategy_from_orders(order_ids, orders_by_id):
    """Determine strategy from order tags"""
    strategy = None
    exit_type = None
    ...
    for oid in order_ids:
        if oid not in orders_by_id:
            continue
        tag = orders_by_id[oid].get("Tag", "").strip()

        # Check for strategy (entry tags)
        ...
        elif tag == "MICRO":  # Generic MICRO tag = normal exit
            if exit_type is None:
                exit_type = "NORMAL_EXIT"
        elif "Stop Market" in orders_by_id[oid].get("Type", ""):
            exit_type = "STOP_MARKET"

    return strategy, exit_type


def find_all_spreads_and_micro(trades, orders_by_id):
    """
    ...
    # Process spread pairs (2 legs with same entry/exit times)
    ...
    for i, trade in enumerate(trades):
        ...
        # Parse order IDs
        order_ids = parse_order_ids(trade.get("Order Ids", ""))
        if not order_ids:
            continue

        # Get strategy and exit type from order tags
        strategy, exit_type = get_strategy_from_orders(order_ids, orders_by_id)

        if strategy in ["ITM_MOMENTUM", "DEBIT_FADE"]:
            # This is a MICRO trade
            ...
            if exit_time.hour == 19 and exit_time.minute >= 25 and not exit_type:
...
# [Rest of the functions remain the same - bucket_debit_width, bucket_pnl_pct, generate_report]
# Copying from v2...
...
def generate_report(spreads, micro_trades, unclassified):
    ...
    if unclassified:
        report.append(f"- **Unclassified:** {len(unclassified)} trade rows\n")
    report.append("\n")
    ...
    if total_micro > 0:
        ...
        for trade in micro_trades:
            strat = trade["strategy"]
            if strat not in strategies_defined:
                strategies_defined[strat] = []
            strategies_defined[strat].append(trade)

        if "ITM_MOMENTUM" in strategies_defined:
            itm_trades = strategies_defined["ITM_MOMENTUM"]
            ...
            itm_avg_loss = (
                sum(t["pnl"] for t in itm_losses_list) / len(itm_losses_list)
                if itm_losses_list
                else 0
            )
        ...
        if "DEBIT_FADE" in strategies_defined:
            fade_trades = strategies_defined["DEBIT_FADE"]
    ...
    if total_vass > 0:
        ...
        if total_micro > 0 and "ITM_MOMENTUM" in strategies_defined:
    ...
    if total_micro > 0:
        ...
        if "ITM_MOMENTUM" in strategies_defined:
            ...
            report.append(
                f"- **Assessment:** {'YES - target met or exceeded' if itm_actual_rr >= 1.7 else 'NO - falling short'}\n\n"
            )
        ...
        if total_vass > 0:
            ...
            if not puts:
                report.append(f"- No PUT spreads in dataset\n\n")
        ...
        if not micro_calls and not micro_puts:
            report.append(f"- No direction data available\n\n")
    ...
    if total_vass > 0 or total_micro > 0:
        ...
        if total_vass > 0:
            ...
            report.append(
                f"1. VASS achieved {vass_actual_rr:.2f}:1 R:R (configured: 1.33:1) - **{('EXCEEDS' if vass_actual_rr > 1.33 else 'BELOW')} target**\n"
            )
            report.append(
        ...
        if total_micro > 0:
            ...
            total_micro_losses = [m for m in micro_trades if not m["is_win"]]
            bug_pct = (
                (len(orphan_losses) / len(total_micro_losses) * 100) if total_micro_losses else 0
            )

            report.append(f"4. MICRO orphan exits accounted for {bug_pct:.0f}% of MICRO losses\n")
            if "ITM_MOMENTUM" in strategies_defined:
                itm_trades = strategies_defined["ITM_MOMENTUM"]
                itm_wins_list = [t for t in itm_trades if t["is_win"]]
                itm_losses_list = [t for t in itm_trades if not t["is_win"]]
                itm_avg_win = (
                    sum(t["pnl"] for t in itm_wins_list) / len(itm_wins_list)
                    if itm_wins_list
                    else 0
                )
                itm_avg_loss = (
                ...
                report.append(
                    f"5. ITM_MOMENTUM achieved {itm_actual_rr:.2f}:1 R:R (configured: 1.8:1) - **{('MEETS TARGET' if itm_actual_rr >= 1.7 else 'BELOW TARGET')}**\n"
                )

    report.append("\n**Data Quality:** All metrics derived from trades.csv and orders.csv. ")
    report.append("Exit reasons inferred from order tags.\n")
...
def main():
    ...
    print(f"Loaded {len(trades)} trades and {len(orders_by_id)} orders")

    print("Classifying trades...")
    spreads, micro_trades, unclassified = find_all_spreads_and_micro(trades, orders_by_id)
    print(
    ...
    print(f"  - Total classified trade rows: {len(spreads)*2 + len(micro_trades)} / {len(trades)}")
