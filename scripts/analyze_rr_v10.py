#!/usr/bin/env python3
"""
V10 R:R Validation Analysis Script
...
def find_spread_pairs(trades):
    """
    Find VASS spread pairs - trades with same entry/exit times
    Returns list of spread dictionaries
    """
    spreads = []
    ...
    # Find pairs (2 legs with same times)
    for key, group in trade_groups.items():
        if len(group) == 2:
            trade1_idx, trade1 = group[0]
            trade2_idx, trade2 = group[1]
                ...
                spreads.append(
                    {
                        ...
                        "pnl_pct_debit": pnl_pct_debit * 100,  # as percentage
...
def find_micro_trades(trades, orders):
    """
    Find MICRO single-leg trades
    """
    micro_trades = []

    # Build order tag lookup
    order_tags = {}
    for order in orders:
        if order.get("Tag"):
            symbol = order["Symbol"]
            time = order["Time"]
            order_tags[(symbol, time)] = order["Tag"]

    for trade in trades:
        # Skip if part of a spread (would be in find_spread_pairs)
        entry_time_str = trade["Entry Time"]
        exit_time_str = trade["Exit Time"]

        # Check if this is a single-leg trade (not paired)
        same_time_count = sum(
            1
            for t in trades
            if t["Entry Time"] == entry_time_str and t["Exit Time"] == exit_time_str
        )

        if same_time_count == 1:  # Single leg
            # Look up strategy from orders
            symbol = trade["Symbols"]
            entry_time = datetime.strptime(entry_time_str, "%Y-%m-%dT%H:%M:%SZ")
            ...
            for order in orders:
                if order["Symbol"] == symbol and order["Time"] == entry_time_str:
                    tag = order.get("Tag", "")
            ...
            # Determine exit type
            exit_time = datetime.strptime(exit_time_str, "%Y-%m-%dT%H:%M:%SZ")
            for order in orders:
                if order["Symbol"] == symbol and order["Time"] == exit_time_str:
                    tag = order.get("Tag", "")
                    if "OCO_STOP" in tag:
                        exit_type = "OCO_STOP"
                    ...
                    elif "Stop Market" in order.get("Type", ""):
                        exit_type = "STOP_MARKET"
                    elif exit_time.hour == 19 and exit_time.minute >= 25:
...
def generate_report(spreads, micro_trades):
    ...
    report.append(
        f"- **Total:** {total_vass + total_micro} trades | ${vass_pnl + micro_pnl:,.0f} net P&L\n\n"
    )
    ...
    # Table B: Exit Reason Distribution (need to infer from logs or times)
    ...
    vass_avg_loss = (
        sum(s["pnl"] for s in vass_losses_list) / len(vass_losses_list) if vass_losses_list else 0
    )
    ...
    # Q1: Are VASS exits clustering at 30% stop?
    ...
    report.append(
        f"- **Finding:** {len(stop_area)} trades hit the stop area, {len(beyond_stop)} trades exceeded it.\n\n"
    )

    # Q2: Are VASS capturing 40% target?
    ...
    report.append(
        f"- **Finding:** {len(big_wins)} trades hit target, {len(mod_wins)} partial, {len(small_wins)} early.\n\n"
    )

    # Q3: Actual vs configured R:R
    ...
    # Q7: CALL vs PUT comparison
    report.append("### Q7: How do CALL vs PUT trades compare on R:R metrics?\n\n")
    report.append("**VASS:**\n")
    if calls and puts:
        call_avg_win = (
            sum(s["pnl"] for s in calls if s["is_win"]) / sum(1 for s in calls if s["is_win"])
        ...
        put_avg_loss = (
            sum(s["pnl"] for s in puts if not s["is_win"]) / sum(1 for s in puts if not s["is_win"])
    ...
    report.append("### VASS Spreads (First 20)\n\n")
    report.append(
    ...
    for i, s in enumerate(spreads[:20]):
    ...
    report.append("### MICRO Trades (First 20)\n\n")
    report.append(
    ...
    for i, m in enumerate(micro_trades[:20]):
    ...
    report.append(
        f"3. {len(stop_area) + len(beyond_stop)} VASS trades ({(len(stop_area) + len(beyond_stop))/len(spreads)*100:.0f}%) hit stops\n"
    )
    report.append(
        f"4. MICRO orphan exits accounted for {bug_pct:.0f}% of MICRO losses (system bugs)\n"
    )
    if "ITM_MOMENTUM" in strategies:
        report.append(f"5. ITM_MOMENTUM achieved {itm_actual_rr:.2f}:1 R:R (configured: 1.8:1)\n")
...
def main():
    ...
    print("Finding VASS spread pairs...")
    spreads = find_spread_pairs(trades)
    print(f"Found {len(spreads)} VASS spreads")

    print("Finding MICRO single-leg trades...")
    micro_trades = find_micro_trades(trades, orders)
    print(f"Found {len(micro_trades)} MICRO trades")

    print("Generating report...")
    report = generate_report(spreads, micro_trades)
