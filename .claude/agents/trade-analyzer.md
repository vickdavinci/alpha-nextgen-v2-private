...
Every analysis folder contains three files:
...
| `*_logs.txt` | Full context: regime, VIX, triggers, micro regime | 30K-50K lines |

---

## CRITICAL: Data Source Priority

1. **trades.csv** is AUTHORITATIVE for: P&L amounts, IsWin flag, entry/exit timestamps, trade counts
2. **orders.csv** provides: Strategy tags (MICRO:ITM_MOMENTUM, VASS:BULL_CALL_DEBIT, etc.)
3. **logs.txt** provides: Regime score, VIX, micro regime, entry signals, exit reasons
...
### Step 0: Parse trades.csv (MANDATORY FIRST)
...
### Step 3: Extract MICRO Context from Logs

For each MICRO trade, find the `INTRADAY_SIGNAL:` log line that generated it.
...
### Step 4: Extract MICRO Exit Trigger from Logs

For each MICRO trade, search logs near the exit time for:
...
### Step 5: Extract VASS Context from Logs

For each VASS spread, find the `SPREAD: ENTRY_SIGNAL` log line.
...
### Step 6: Extract VASS Exit Trigger from Logs

For each VASS spread, search logs near the exit date for:
...
1. **ZERO N/A for MICRO context columns** where an INTRADAY_SIGNAL line exists in the logs. Search by date, then by direction (CALL/PUT), then by approximate time. If truly not found, write `NOT_FOUND (searched YYYY-MM-DD for CALL/PUT)`.
2. **ZERO N/A for VASS context columns** where a SPREAD: ENTRY_SIGNAL line exists. Search by entry date. If not found, write `NOT_FOUND (searched YYYY-MM-DD)`.
...
- **Missing INTRADAY_SIGNAL for a MICRO trade:** Search ±2 hours from entry time. Search by date + direction. If still not found, use the nearest REGIME log line for regime score and VIX.
- **Missing SPREAD: ENTRY_SIGNAL for a VASS trade:** Search the entire day. Try `VASS_ENTRY:` as fallback pattern.
...
3. Context columns (regime, VIX, exit trigger) are populated, not N/A
