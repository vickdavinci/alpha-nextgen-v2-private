...
class TestNeutralityExit:
    ...
    def test_bull_debit_day6_thesis_exit_for_broken_trade(
        self, engine, long_leg, short_leg, monkeypatch
    ):
        """Day-6 thesis review should exit clearly broken bull debits."""
        from types import SimpleNamespace
        ...
        engine.algorithm = SimpleNamespace(
            Time=datetime(2027, 1, 7, 15, 45, 0),
            Portfolio=SimpleNamespace(TotalPortfolioValue=100000.0),
            qqq_sma20=SimpleNamespace(IsReady=True, Current=SimpleNamespace(Value=100.5)),
            Log=lambda *_args, **_kwargs: None,
        )
        monkeypatch.setattr(config, "VASS_BULL_DEBIT_DAY6_THESIS_REVIEW_ENABLED", True)
        monkeypatch.setattr(config, "VASS_BULL_DEBIT_DAY6_THESIS_REVIEW_MIN_HOLD_DAYS", 6)
        monkeypatch.setattr(config, "VASS_BULL_DEBIT_DAY6_THESIS_REVIEW_TIME", "15:45")
        monkeypatch.setattr(config, "VASS_BULL_DEBIT_DAY6_THESIS_EXIT_DRAWDOWN_PCT", 0.015)
        monkeypatch.setattr(config, "VASS_BULL_DEBIT_DAY6_THESIS_EXIT_MAX_PROGRESS_PCT", 0.10)
        ...
        monkeypatch.setattr(config, "VASS_ENABLE_TAIL_CAP_EXITS", False)
        monkeypatch.setattr(config, "VASS_ENABLE_DAY4_EOD_EXITS", True)
        ...
        result = engine.check_spread_exit_signals(
            ...
            vix_current=18.0,
            current_dte=25,
            underlying_price=98.4,
        )

        assert result is not None
        assert "DAY6_THESIS_EXIT" in str(result[0].reason)

    def test_bull_debit_day6_thesis_review_revokes_thesis_and_keeps_trade_open(
        self, engine, long_leg, short_leg, monkeypatch
    ):
        """Day-6 review should downgrade weak but not broken thesis trades."""
        from types import SimpleNamespace
        ...
        assert result is None
        assert engine._spread_position.thesis_mode_revoked is True

    def test_bull_debit_day4_cleanup_is_skipped_in_favor_of_day6_review(
        self, engine, long_leg, short_leg, monkeypatch
    ):
        """Bull call debits should no longer fire the blunt day-4 EOD cleanup."""
        from types import SimpleNamespace
        ...
        monkeypatch.setattr(config, "VASS_ENABLE_DAY4_EOD_EXITS", True)
        monkeypatch.setattr(config, "VASS_DAY4_EOD_DECISION_ENABLED", True)
        monkeypatch.setattr(config, "VASS_DAY4_EOD_KEEP_IF_PNL_GT", -0.20)
        monkeypatch.setattr(config, "SPREAD_NEUTRALITY_EXIT_ENABLED", False)
        monkeypatch.setattr(config, "VASS_REGIME_BREAK_EXIT_ENABLED", False)

        result = engine.check_spread_exit_signals(
            long_leg_price=4.30,
            short_leg_price=2.30,
            regime_score=54.0,
            vix_current=18.0,
            current_dte=25,
...
def test_spread_position_roundtrip_preserves_thesis_revocation(sample_contract):
    """V12.39: SpreadPosition persistence retains thesis downgrade state."""
    spread = SpreadPosition(
        long_leg=sample_contract,
        short_leg=sample_contract,
    ...
    restored = SpreadPosition.from_dict(spread.to_dict())
    assert restored.thesis_mode_revoked is True
...
class TestBearPutProfitTargetScoping:
    """Tests for the V12.31 BEAR_PUT-only profit-target scoping fix."""
    ...
    def _make_credit_spread(self) -> SpreadPosition:
        """Reuse the standard credit-spread fixture shape for nearby OGP regressions."""
        long_leg = OptionContract(
            symbol="QQQ 240816C00457000",
            underlying="QQQ",
            direction=OptionDirection.CALL,
    ...
    def test_bear_put_profit_target_ignores_intrinsic_override(self, engine, monkeypatch):
        ...
        monkeypatch.setattr(config, "VASS_ENABLE_PROFIT_TARGET_EXITS", True)
        monkeypatch.setattr(config, "SPREAD_NEUTRALITY_EXIT_ENABLED", False)
        monkeypatch.setattr(config, "VASS_ENABLE_TAIL_CAP_EXITS", False)

        result = engine.check_spread_exit_signals(
            long_leg_price=10.38,
            short_leg_price=9.50,
            regime_score=40.0,
            vix_current=20.0,
            current_dte=34,
    ...
    def test_bull_call_profit_target_still_uses_existing_debit_path(self, engine, monkeypatch):
        ...
        result = engine.check_spread_exit_signals(
            ...
            vix_current=14.0,
            current_dte=34,
            underlying_price=292.75,
        )

        assert result is not None
        assert "PROFIT_TARGET" in str(result[0].reason)

    def test_bear_put_mfe_and_trail_ignore_intrinsic_override(self, engine, monkeypatch):
        """BEAR_PUT trail/MFE tracking must not arm from intrinsic-only phantom gains."""
        long_put = OptionContract(
            symbol="QQQ 271231P00293000",
            underlying="QQQ",
            direction=OptionDirection.PUT,
        ...
        assert result is None
        assert spread.highest_pnl_pct < 0.20
